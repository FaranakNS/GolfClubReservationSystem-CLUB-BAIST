...
namespace GolfClubReservationSystem.Pages
{
    public class UpdateTeeTimeModel : PageModel
    {
        ...
        public UpdateTeeTimeModel(IConfiguration configuration)
        {
        ...
        [BindProperty]
        [Range(0, 4, ErrorMessage = "Number of players must be between 0 and 4.")]
        ...
        public void OnPost(string submit)
        {
            ...
            if (submit == "FetchDetails")
            {
                ...
                try
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        IsBookingFound = true;
                        Message = "Booking details fetched. You can now update or delete.";
                    }
                    else
                    {
            ...
            else if (submit == "UpdateBooking")
            {
                ...
                try
                {
                    ...
                    Message = rowsAffected > 0
                        ...
                        : "Update failed. Booking ID might be incorrect or data unchanged.";
                }
                catch (Exception ex)
                {
            ...
            else if (submit == "DeleteBooking")
            {
                if (BookingId <= 0)
                {
                    Message = "Invalid Booking ID. Cannot delete.";
                    return;
                }
                ...
                {
                    using SqlConnection connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
                    string query = "DELETE FROM BookTeeTime WHERE BookingId = @BookingId";
                    ...
                    {
                        Message = "Delete failed. Booking may not exist.";
                    }
                }
                catch (Exception ex)
                {
